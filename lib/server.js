...
server
	...
	.set('views', './lib/views' )
	.use(setResLocals)
	.get('/', (req, res, next) => res.redirect(`${req.baseUrl}/context?mapping=buurten&day=total`))
...
async function renderHome(req, res) {
	...
	try {
		...
		if(datasets.length > 0) {
			featureCollection = await chartsData.mapData(mapping);
			categoryData = await chartsData.amsData();
		} else {
