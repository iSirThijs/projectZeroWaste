...
server
	...
	.set('views', './lib/views' )
	.use(setResLocals)
	.get('/', (req, res, next) => res.redirect('/context?mapping=stadsdelen&day=total'))
...
async function renderHome(req, res) {
	const { mapping, day } = req.query;

	try {
		// let data = await area.get(mapping);
		let featureCollection = await amsAPI.getFeatureCollection(mapping);
