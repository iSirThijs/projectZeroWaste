...
function countLoadedDatasets(){
	return Job.findByStatus('done');
}
...
async function extractEntries(job, data){
	...
	for (let current = 0; current < data.length; current++) {
		logger.silly(`Current: ${current + 1} out of ${data.length}`);
		...
		try {
			...
			if(data.length === buildings.length) return buildings;
		} catch(error) {
			logger.error(error);
			throw error;
		}
